...
- **Writing a new spec:** copy the `<head>` from any existing file in `specs/`, link the shared stylesheet (`<link rel="stylesheet" href="spec.css">`), write semantic HTML, and give it a `<p class="lede">` — the prune uses it as the one-line summary. **Put `<p><a href="index.html">← Specs index</a></p>` just inside `<body>`** (inside `<div class="wrap">` where there is one) — a spec is usually opened on its own, and one with no way back is a dead end. `./specs.sh check` names any spec missing it. Declare at minimum `spec-status`; add `spec-touches` (files it edits, for conflict detection and ordering), `spec-needs` (slugs that must land first), `spec-blocker` (one dated line of what remains), `spec-decision` (one per owner choice), and `spec-verify` (a shell command that exits 0 when done). The full table is in the folded **Reference** at the bottom of `specs/index.html`.
...
- **Every decision the owner makes gets written into the owning spec, in the same turn they make it.** Not summarised in chat and left there — chat is not the archive, and a decision that lives only in a conversation gets re-argued in three weeks by someone with no access to it. Record the answer, the date, and the owner's own words where they are short enough to quote. **When the answer reverses what the spec recommended, mark the old passage superseded rather than deleting it** and say what changed: the reasoning is usually still sound and only the conclusion moved, and a guardrail written for the old conclusion will otherwise be followed faithfully into undoing the new one. Then `./specs.sh prune`, which drops it from the dashboard.
