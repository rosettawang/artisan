#
# ABOUT
# Artisan Standalone Designer Tool
# 
# LICENSE
...
# AUTHOR
# Created for standalone roast profile design
...
try:
    ...
    from PyQt6.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, 
        QHBoxLayout, QGridLayout, QGroupBox, QLabel, QLineEdit, QComboBox, 
        QCheckBox, QPushButton, QMessageBox, QFileDialog, QListWidget, 
        QListWidgetItem, QSpinBox, QDoubleSpinBox, QTextEdit)
...
class DesignerData:
    ...
    def load_defaults(self, factory_defaults: Dict[str, Dict[str, Any]]) -> Dict[str, Dict[str, Any]]:
        ...
        try:
            # Try to import QSettings - it might not be available during data-only operations
            try:
    ...
    def export_alog(self, filename: str) -> None:
        """Export profile as Artisan .alog file"""
        try:
            ...
            # Create Artisan alog data structure
            alog_data = {
                "version": "2.10.0",
                ...
                "ground_color": 0,
                "whole_color": 0,
                "roastingnotes": "Generated by Artisan Profile Designer",
                "cupping_notes": "",
                "ambientTemp": 20.0,
                "ambientHumidity": 50.0,
                "pressure": 1013.25,
                "elevation": 0.0,
                "timex": time_curve.tolist(),
                "temp1": et_curve.tolist(),  # ET
                "temp2": bt_curve.tolist(),  # BT
                "timeindex": [0, 0, 0, 0, 0, 0, 0],  # Will be updated with landmark indices
                "extradevices": [],
                "extratimex": [],
                "extratemp1": [],
                "extratemp2": [],
            ...
            for i, landmark_name in enumerate(landmark_order):
                if landmark_name in enabled_landmarks:
                    landmark_time = enabled_landmarks[landmark_name]['time']
                    # Find closest time index
                    time_idx = np.argmin(np.abs(time_curve - landmark_time))
                    alog_data["timeindex"][i] = int(time_idx)
                else:
                    # Set to -1 for disabled landmarks (Artisan standard)
                    alog_data["timeindex"][i] = -1
            
            # Add events
            for event in self.events:
                time_idx = np.argmin(np.abs(time_curve - event['time']))
            ...
            for alarm in self.alarms:
                time_idx = np.argmin(np.abs(time_curve - alarm['time']))
                alog_data["specialevents"].append(int(time_idx))
                alog_data["specialeventstype"].append(1)  # Alarm event type
                alog_data["specialeventsvalue"].append(alarm['temperature'])
                alog_data["specialeventsStrings"].append(f"ALARM {alarm['temp_type']} {alarm['temperature']}°C ({alarm['action']})")
            
            # Write alog file
            with open(filename, 'w') as f:
                json.dump(alog_data, f, indent=1)
                
        except Exception as e:
            raise Exception(f"Failed to export .alog file: {e}")
...
def stringfromseconds(seconds: float) -> str:
...
def stringtoseconds(time_str: str) -> float:
...
class StandaloneDesignerWindow(QMainWindow):
    ...
    def create_landmarks_group(self) -> QGroupBox:
        ...
        for name, data in self.data.landmarks.items():
            ...
            # Time input
            time_edit = QLineEdit(stringfromseconds(data['time']))
    ...
    def apply_landmark_changes(self):
        ...
        try:
            ...
            for name, widgets in self.landmark_widgets.items():
                ...
                try:
                    time_seconds = stringtoseconds(time_str)
    ...
    def update_landmark_time(self, name: str, time_str: str):
        """Update landmark time (without plot update)"""
        try:
            time_seconds = stringtoseconds(time_str)
            self.data.landmarks[name]['time'] = time_seconds
        except:
    ...
    def update_landmark_bt(self, name: str, temp_str: str):
        """Update landmark BT temperature (without plot update)"""
        try:
            temp = float(temp_str)
            self.data.landmarks[name]['BT'] = temp
        except:
            pass
            
    def update_landmark_et(self, name: str, temp_str: str):
        """Update landmark ET temperature (without plot update)"""
        try:
            temp = float(temp_str)
            self.data.landmarks[name]['ET'] = temp
        except:
            pass
    ...
    def refresh_events_list(self):
        ...
        for i, event in enumerate(self.data.events):
            time_str = stringfromseconds(event['time'])
            desc = event['description'] if event['description'] else event['type']
    ...
    def refresh_alarms_list(self):
        ...
        for i, alarm in enumerate(self.data.alarms):
            time_str = stringfromseconds(alarm['time'])
    ...
    def refresh_ui(self):
        ...
        for name, widgets in self.landmark_widgets.items():
            ...
            widgets['time'].setText(stringfromseconds(data['time']))
